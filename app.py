...
st.markdown(
    "Select a country to view the chart of total energy generation,\
...
country = st.sidebar.selectbox(
    label="Select a Country", index=9, options=list(COUNTRY_MAPPINGS.keys())
)
...
forecast_horizon = st.sidebar.slider(
    label="Forecast Horizon (hours)", min_value=12, max_value=168, value=48
)
...
for item in df.columns:
    ...
    df_forecast = generate_forecast(
        df[[item]], regressor, forecast_horizon, window_length
    )
